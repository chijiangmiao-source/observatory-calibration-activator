...
- **逐块检查点**：每块先验 SHA-256，通过后才以**独立事务**写入 IndexedDB
  （`chunks` 仓库）。
- **故障注入**：界面可设定“第 N 块提交后刷新页面”，模拟供电切换；编号超出
  分块范围（如单块包设为第 1 块）会被拒绝并明确提示该编号不存在。
...
- **损坏报告**：载荷分块损坏按最小编号报告；重复安装同一版本时会对所选载荷
  完整复验（损坏同样按最小编号报告，同版本不同内容拒绝覆盖），且不会产生
  第二份记录（`packages` 以 `version` 为主键）。
